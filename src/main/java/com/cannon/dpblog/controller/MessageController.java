...
import com.cannon.dpblog.service.MessageService;
...
public class MessageController implements CommunityConstant {
    ...
    public String getNoticeList(Model model) {
        ...
        Map<String, Object> messageVO = getMessageVo(message,user,TOPIC_COMMENT,true);
        if(messageVO!=null){
            unread += (Integer) messageVO.get("unread");
        }
        model.addAttribute("commentNotice", messageVO);

        // 查询点赞类通知
        message = messageService.findLatestNotice(user.getId(), TOPIC_LIKE);
        messageVO = getMessageVo(message,user,TOPIC_LIKE,true);
        if(messageVO!=null){
            unread += (Integer) messageVO.get("unread");
        }
        ...
        messageVO = getMessageVo(message,user,TOPIC_FOLLOW,false);
        if(messageVO!=null){
            unread += (Integer) messageVO.get("unread");
        }
        ...
        // 查询未读通知数量
//        int noticeUnreadCount = messageService.findNoticeUnreadCount(user.getId(), null);
        model.addAttribute("noticeUnreadCount", unread);
    ...
    private Map<String,Object> getMessageVo (Message message,User user,String topic,boolean isPost){
